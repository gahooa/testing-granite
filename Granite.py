...
try:
  ...
  def _():
    oldsplit = postgresql.versionstring.split
    def newsplit(vstr):
      return oldsplit(vstr.split()[0])
    postgresql.versionstring.split = newsplit
  _()
  del _
...
class DT():
  ...
  ISO_DATE_MATCH = re.compile(r'^([0-9]{4})-([0-9]{2})-([0-9]{2})$').match
  ...
  @classmethod
  def GetSunSat(cls, /, thedate):
    '''
    Returns a tuple of (datetime.date, datetime.date) for the start and end of the week where the week is defined as
    Sunday->Saturday
    '''
    start = thedate - datetime.timedelta(days=thedate.weekday()+1)
    end = start + datetime.timedelta(days=6)
    return start, end

  @classmethod
  def ISOTextToDate(cls, /, datestring):
    m = cls.ISO_DATE_MATCH(datestring)
    if not m:
      raise ValueError(f'Date is not a valid ISO8601 string (YYYY-MM-DD): {datestring}')
    g = m.groups()
...
class AS3():
  ...
  def Struct_Date(self, StructPath, StructIn, Struct):
    Struct['+MaxValue'] = DATEN(StructIn.pop('+MaxValue', None))
    Struct['+MinValue'] = DATEN(StructIn.pop('+MaxValue', None))

  def Python_Date(self, StructPath, Struct, VarDepth, Prefix, Lines, Opt):
    Lines.append(Prefix + f'vo{VarDepth} = DT.ISOTextToDate(vi{VarDepth})')    
...
if redis:
  class Redis(redis.Redis):
    ...
    def set_bool(self, key, value, **kwargs):
      return self.set(key, str(int(bool(value))), **kwargs)

    def get_bool(self, key):
      value = self.get(key)
      return None if value is None else bool(int(value))

    def set_int(self, key, value, **kwargs):
      return self.set(key, str(int(value)), **kwargs)
    ...
    def set_str(self, key, value, **kwargs):
      return self.set(key, str(value).encode('utf-8'), **kwargs)
    ...
    def set_json(self, key, value, **kwargs):
      return self.set(key, JSON_Encode(value).encode('utf-8'), **kwargs)
